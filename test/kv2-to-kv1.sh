...
## Run App
value_must_be $(${APP_BIN} -c "ls secretkv2/source" | wc -l) "4" &&
value_must_be $(${APP_BIN} -c "grep ${VAULT_TEST_VALUE} secretkv2/source" | wc -l) "5" &&
${APP_BIN} -c "cp secretkv2/source/x secretkv1/target2/x" &&
${APP_BIN} -c "mv secretkv2/source/ secretkv1/target/" &&
value_must_be $(${APP_BIN} -c "ls secretkv1/target" | wc -l) "4" &&
value_must_be $(${APP_BIN} -c "grep ${VAULT_TEST_VALUE} secretkv1/target" | wc -l) "5" &&
